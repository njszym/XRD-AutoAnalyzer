    ...
    if (struct_A is None) or (struct_B is None): ## If a good match could not be made, ignore this pair
        ...
        for (A, B) in zip(A_species, B_species):
            if A == B:
                ...
                interp_structs[3][index] = mg.PeriodicSite.from_dict(site_dict)
            index += 1
        phase_filenames = ['%s_%s.cif' % (struct.composition.reduced_formula, struct.get_space_group_info()[1]) for struct in interp_structs[1:]]
        return interp_structs[1:], phase_filenames ## 25%, 50%, 75% compositions along with their filenames denoted composition_spacegroup.cif
