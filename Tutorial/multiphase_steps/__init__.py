...
class SpectrumAnalyzer(object):
    ...
    def enumerate_routes(self, spectrum, indiv_conf=[], indiv_pred=[], confidence_list=[], prediction_list = [], is_first=True):
        ...
        y = np.array(spectrum).flatten()
        plt.plot(x, y, 'b-')
        if is_first:
            plt.title('Initial spectrum')
        else:
            plt.title('Spectrum after subtracting known peaks')
        plt.show()

        prediction, num_phases, certanties = self.kdp.predict(spectrum)
        num_phases = 1 # Simplified for tutorial - only investigate most probable phase
    ...
    def get_reduced_pattern(self, predicted_cmpd, orig_y, last_normalization=1.0):
        ...
        x = np.linspace(10, 80, 4501)
        plt.plot(x, orig_y, 'b-', label='Spectrum in question')
        plt.plot(x, warped_spectrum, 'r-', label='Peaks of identified phase')
        plt.legend()
        plt.show()
